...
def Neg_gru(dataname, device, train_x, train_y, test_x, test_y, args):
    st = time.time()
    if args.t == 0:
...
def Val(net, test_x, hg_test, test_y, maxauc, maxpr,proto,maxaucs,dataname):
    with torch.no_grad():
        ...
        if auc > maxaucs:
            ...
            net.trainmodel = False
            with torch.no_grad():
                torch.save(net, f'./Well_Trained_Models/Absolution/Neg_gru/{dataname}.pt')
                torch.save(proto, f'./Well_Trained_Models/Absolution/Neg_gru/prototypes/{dataname}.pt')
...
class ModelP(nn.Module):
    ...
    def forward(self, X, hg):
        ...
        z = torch.sigmoid(self.fc1_update(x_e) + self.fc2_update(x_e1))
        r = torch.sigmoid(self.fc1_reset(x_e) + self.fc2_reset(x_e1))

        out = torch.tanh(self.fc1(x_e) + self.fc2(r * x_e1))
