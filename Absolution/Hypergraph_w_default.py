...
def Hypergraph_w_default(dataname, device, train_x, train_y, test_x, test_y, args):
    st = time.time()
    if args.t == 0:
...
def Val(net, test_x, hg_test, test_y, maxauc, maxpr,proto,maxaucs,dataname):
    with torch.no_grad():
        ...
        if auc > maxaucs:
            ...
            net.trainmodel = False
            with torch.no_grad():
                torch.save(net, f'./Well_Trained_Models/Absolution/Hypergraph_w_default/{dataname}.pt')
                torch.save(proto, f'./Well_Trained_Models/Absolution/Hypergraph_w_default/prototypes/{dataname}.pt')
