...
class AutomaticWeightedLoss(torch.nn.Module):
    ...
    def forward(self, *x):
        ...
        return loss_sum
def N_prototype(dataname, device, train_x, train_y, test_x, test_y, args):
...
def train(dataname,train_x, train_y, test_x, test_y, hg_train, hg_test,num_embedding, num_features,device,args):
    ...
    optimizer = opt(net)
    loss_module = AutomaticWeightedLoss(num=2)
    ...
    for epoch in range(200):
        ...
        L = loss_module.forward(reconloss, dis)

        L.backward()
        optimizer.step()


        auclist, prlist= Val(net, test_x, hg_test, test_y,  auclist, prlist, pro,maxaucs,dataname)
...
def Val(net, test_x, hg_test, test_y, maxauc, maxpr,proto,maxaucs,dataname):
    with torch.no_grad():
        ...
        error = torch.mean((test_x - outs).pow(2), dim=1).data
        ...
        if auc > maxaucs:
            ...
            net.trainmodel = False
            with torch.no_grad():
                torch.save(net, f'./Well_Trained_Models/Absolution/N_prototype/{dataname}.pt')
...
def opt(net):
    opt = optim.Adam([
        ...
        {'params': net.discrimtor.parameters(), 'lr': 0.00001} #原始为0.00001
    ], lr=0.01)
    return opt
